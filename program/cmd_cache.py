...
class CmdCache( ):
    ...
    def addListener(self, file, listener, config):
        ...
        timeout   = listener.get('TIMEOUT')  or config.get('TIMEOUT')
        if variables:
        ...
        return self.add( file, cmd, dir, out, env, timeout )

    def add(self, file, cmd, dir, out, env, timeout):
        out = 'OnSave Console ['+str(out)+']' if out else 'Background'
        cmd = { "cmd" : cmd,  "out" : out, "file" : file, "dir" : dir,  "env" : env, "timeout" : timeout }
